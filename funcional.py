    ...
    try:
        ...
        for index, linha in dt.iterrows(): # Le linha a linha da planilha  
            ...
            if proxima_key != key and proxima_key is not None: # Verifica se a proxima key é diferente e se não é None
                ...
                print("SALVO")
                time.sleep(2)
                posicao = auto.locateOnScreen('MENSAGEM.png', confidence=0.9) # Localize a posição da imagem na tela
                if posicao is not None: # Verifique se a imagem foi encontrada
                    time.sleep(15)
                    ...
                    if msg_erro == "":
                        ...
                        auto.hotkey('ctrl', 'c')
                        time.sleep(1)
                        msg_erro = pyperclip.paste() # Divide o texto em linhas e seleciona as três primeiras linhas
